import { CategoryCreateDTO, updateCategoryDTO } from "@/types/category";
...
export const updateCategory = async (updateCategoryDTO: updateCategoryDTO) => {
  const response = await fetch(`${CATEGORY_ENDPOINT}/${updateCategoryDTO.id}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
